    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        for y in (0..self.dim[1]).rev() {
            for x in 0..self.dim[0] {
                let pnt: Point = Point::from([x as i64, y as i64]);
                chars.push(char::from_digit(self.values[&pnt], 10).unwrap());
            }
            chars.push('\n');
        }
